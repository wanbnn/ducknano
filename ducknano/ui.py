...
def render_dashboard(hist_enabled: bool = False):
    ...
    provider_table.add_row("[bold #00ffcc]Provider[/bold #00ffcc]", escape(provider_name()))
    provider_table.add_row("[bold #00ffcc]Model[/bold #00ffcc]", escape(PROVIDER_CONFIG.get("model") or "auto"))
    provider_table.add_row("[bold #00ffcc]API key[/bold #00ffcc]", escape(masked_api_key()))
    provider_table.add_row("[bold #00ffcc]Temperature[/bold #00ffcc]", escape(str(PROVIDER_CONFIG.get("temperature", "off"))))

    workspace_table = Table(show_header=False, box=None, padding=(0, 1))
    workspace_table.add_row("[bold #00ffcc]Base URL[/bold #00ffcc]", escape(PROVIDER_CONFIG.get("base_url", "")))
    workspace_table.add_row("[bold #00ffcc]History[/bold #00ffcc]", "on" if hist_enabled else "off")
    workspace_table.add_row("[bold #00ffcc]Setup[/bold #00ffcc]", "use [bold]/setup[/bold] to configure")
    workspace_table.add_row("[bold #00ffcc]Help[/bold #00ffcc]", "use [bold]/help[/bold] for commands")

    left = Panel(provider_table, title="Connection", border_style="#00ffcc", box=box.ROUNDED)
    right = Panel(workspace_table, title="Session", border_style="#ff55ff", box=box.ROUNDED)
...
def render_saved_provider():
    ...
    table.add_row("[bold #00ffcc]Provider[/bold #00ffcc]", escape(provider_name()))
    ...
    table.add_row("[bold #00ffcc]API key[/bold #00ffcc]", escape(masked_api_key()))
    table.add_row("[bold #00ffcc]Temperature[/bold #00ffcc]", escape(str(PROVIDER_CONFIG.get("temperature", "off"))))
